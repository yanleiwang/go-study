...
func TestDelayQueue_Dequeue(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			q:       NewDelayQueueUseSema[delayElem](10),
			timeout: time.Second,
			wantErr: context.DeadlineExceeded,
		},
		{
	...
	t.Run("dequeue while enqueue", func(t *testing.T) {
		q := NewDelayQueueUseSema[delayElem](3)
		go func() {
	...
	t.Run("dequeue two elements concurrently with larger delay intervals", func(t *testing.T) {
		...
		capacity := 2
		q := NewDelayQueueUseSema[delayElem](capacity)
...
func newDelayQueue(t *testing.T, eles ...delayElem) *DelayQueueUseSema[delayElem] {
	q := NewDelayQueueUseSema[delayElem](len(eles))
